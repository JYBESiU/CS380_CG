...
void onMouseDrag( int x, int y ) {
    ...
    if (transMode == 'm') {
        ...
        glLoadIdentity();
        glMultMatrixd(cow2wld.matrix());
        glTranslated(amount[0] * (x - trans_old), amount[1] * (x - trans_old), amount[2] * (x - trans_old));
        glGetDoublev(GL_MODELVIEW_MATRIX, cow2wld.matrix());
        glPopMatrix();
        trans_old = x;
    }
        ...
        glLoadIdentity();
        if (moveOrRotate) {
            glTranslated(cow2wld.matrix()[12] - cameras[cameraIndex][0], cow2wld.matrix()[13] - cameras[cameraIndex][1], cow2wld.matrix()[14] - cameras[cameraIndex][2]);
            ...
            glMultMatrixd(cam2wld[cameraIndex].matrix());
            glTranslated(amount[0] * (x - trans_oldX), amount[1] * (y - trans_oldY), amount[2] * (x - trans_oldX));
            glMultMatrixd(wld2cam[cameraIndex].matrix());
            glMultMatrixd(cow2wld.matrix());
        }
        glGetDoublev(GL_MODELVIEW_MATRIX, cow2wld.matrix());
